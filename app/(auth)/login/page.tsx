    ...
    <main className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-white rounded-[40px] shadow-2xl p-10 border border-slate-100">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button
            disabled={isLoading}
            className="w-full bg-otica-roxo text-white font-black py-5 rounded-2xl flex items-center justify-center gap-3 hover:opacity-90 transition-all uppercase text-xs tracking-widest disabled:opacity-50"
          >
            {isLoading ? (
        ...
        <p className="mt-8 text-center text-slate-400 text-xs font-medium">
          Ainda não é cliente?{" "}
          <Link href="/register" className="text-otica-roxo font-black">
            Cadastre-se
          </Link>
        </p>
      </div>
