...
import { MessageCircle, MapPin, Glasses, Sun, BookOpen, Instagram, Facebook } from "lucide-react";
    ...
    <main className="min-h-screen bg-[#F8F9FA] font-sans text-slate-900">
      ...
      <header className="bg-white border-b border-slate-100 py-4 px-6 flex justify-between items-center sticky top-0 z-50">
        ...
        <div className="flex gap-4 items-center">
          <a href="#" className="text-slate-400 hover:text-otica-roxo transition-colors"><Instagram size={20} /></a>
          <a href="#" className="text-slate-400 hover:text-otica-roxo transition-colors"><Facebook size={20} /></a>
        ...
        <div className="container mx-auto px-6 relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
          <div className="max-w-xl text-center md:text-left">
            <h1 className="text-4xl md:text-6xl font-black text-white leading-[1.1] mb-6">
              Modelos exclusivos para <span className="text-otica-amarelo">todos os estilos.</span>
            </h1>
      ...
      <section className="py-20 container mx-auto px-6">
        <h2 className="text-center text-slate-400 uppercase tracking-[0.2em] text-xs font-bold mb-12">O que você procura?</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {[
            ...
            <div key={index} className="flex flex-col items-center group cursor-pointer">
              ...
              <span className="font-bold text-slate-700 group-hover:text-otica-roxo transition-colors">{item.label}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-white py-24 border-y border-slate-100">
        <div className="container mx-auto px-6">
          <div className="text-center max-w-2xl mx-auto mb-16">
            <h3 className="text-3xl font-black text-otica-roxo mb-4">Encontre uma Ótica Popular</h3>
            <p className="text-slate-500 font-medium">Estamos estrategicamente localizados para melhor lhe atender em Parnaíba.</p>
          </div>
          
          <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            <CardUnidade nome="Unidade Centro" cidade="Rua Exemplo, 123 - Parnaíba - PI" />
            <CardUnidade nome="Unidade Loja 02" cidade="Av. São Sebastião, 456 - Parnaíba - PI" />
          </div>
        </div>
      </section>

      <footer className="py-12 text-center text-slate-400 text-sm">
        <p className="font-bold text-otica-roxo/50 mb-2">Ótica Popular © Parnaíba-PI</p>
